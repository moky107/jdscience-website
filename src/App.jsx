...
/* ============================================================
   jdscience.co.uk — Teal Classic (photo top, video bottom)
        ...
        <nav style={{ background: "#ecfeff", borderTop: "1px solid rgba(0,0,0,0.04)", padding: "8px 0", maxHeight: "72vh", overflowY: "auto" }}>
          ...
          <div style={{ padding: "0 18px 12px" }}>
            <button type="button" onClick={() => { handleAdmin(); setMenuOpen(false); }}
              style={{ width: "100%", padding: "10px 12px", borderRadius: 8, border: "1px solid #e6e6e6", background: isAdmin ? "#10b981" : "#fff", color: isAdmin ? "#fff" : "#111" }}>{isAdmin ? "Admin: ON" : "Admin"}</button>
          </div>
        </nav>
      )}
    </header>
...
function SubjectCards({ onPick }) {
  ...
  return (
    <section style={{ padding: isMobile ? "32px 16px" : "48px 20px", background: "#f8fafc" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        <h2 style={{ textAlign: "center", color: "#0f172a", fontSize: isMobile ? 24 : 28 }}>Browse by Subject</h2>
        <p style={{ textAlign: "center", color: "#64748b", marginTop: 4 }}>Past papers, notes & mark schemes for every exam board</p>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 16, marginTop: 26 }}>
          {SUBJECTS.map(s => (
  ...
  return (
    <section style={{ padding: isMobile ? "20px 14px" : "28px 20px", background: "#f8fafc", minHeight: "60vh" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <h2 style={{ color: "#0f172a", marginBottom: 16, fontSize: isMobile ? 19 : 24 }}>{activeLevel} {activeSubject} — {activeRes} by Exam Board</h2>
